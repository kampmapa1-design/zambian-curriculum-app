...
/// Timetable Generation, Stage 1 — "Data Setup". Periods/day, period
/// length, teaching days/week, an editable per-subject "suggested
/// default" periods-per-week value (never presented as an official
/// figure — labelled as adjustable everywhere it appears), and the
/// editable Practical Subjects Exception List (subjects allowed to be
/// scheduled as a single period; everything else defaults to a
/// continuous 2-period block once Stage 4's engine exists to apply it).
class TimetableSetupScreen extends StatefulWidget {
  const TimetableSetupScreen({required this.school, super.key});
  final School school;
...
class _TimetableSetupScreenState extends State<TimetableSetupScreen> {
  ...
  Future<void> _load() async {
    ...
    final saved = await _timetableService.getConfig(widget.school.id);
    final classSubjects = await _timetableService.distinctSubjectsAcrossClasses(widget.school.id);
  ...
  Future<void> _generate() async {
    // Save first so the engine always runs against exactly what's on
    // screen, not a possibly-stale saved copy.
    await _save();
    if (!mounted) return;
    setState(() => _generating = true);
    try {
      final result = await _timetableService.generate(widget.school.id);
      if (!mounted) return;
      unawaited(FreeTierEntitlementService.instance.recordUsed(FreeTierFeature.timetableGeneration));
      await Navigator.of(context).push(
        MaterialPageRoute(builder: (_) => GeneratedTimetableScreen(school: widget.school)),
      );
      if (!mounted) return;
          ...
          : ListView(
              ...
              children: [
                ...
                const Padding(
                  ...
                  child: Text('Suggested default — adjust as needed for your school.', style: TextStyle(fontSize: 12, fontStyle: FontStyle.italic)),
                ),
                ...
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: 4),
                  child: Text(
                    'Runs a deterministic engine (no AI) that places every connected class\'s subjects into slots, '
                    'respecting teacher availability, no double-booking, the double-period rule above, and the max '
                    'daily load. Anything it can\'t resolve is listed as a specific conflict, never silently skipped.',
